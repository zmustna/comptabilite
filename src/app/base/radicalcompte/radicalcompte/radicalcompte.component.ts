...
@Component({
  selector: 'app-radicalcompte',
  templateUrl: './radicalcompte.component.html',
  styleUrls: ['./radicalcompte.component.scss']
})
export class RadicalcompteComponent implements OnInit {
  cols = [
    ...
    { field: 'compte', header: 'Compte', width: '20%', display: 'table-cell',  align: 'left', size: '0.8em',background: 'black',color: 'white'},
  ];
...
this.displaycompte = true;
  ...
  selectrow(e) {
      this.angForm.setValue({ name: e.data.libelle, compte: e.data.compte });
  ...
  onDelete(){
   ...
   res.push('id'); res.push(this.codeid);
    this.resultx = []; this.resultx = res;
   this.Spinner.show();
     this.service.getList("/RadicalC/GetDelRadicalC", this.resultx)
   .subscribe(
     (data:any) => {
  ...
  onSubmit() {
    this.loading = true; let fiile = new radical(); let i = 0;
      fiile.libelle = this.angForm.value.name;
      fiile.compte = this.angForm.value.compte;
       fiile.type_code = this.codeid;

      this.service.ExecutePost("/RadicalC/Update", fiile).subscribe(() => {
      if (i === 0) {
  ...
  getstate(){
    this.Spinner.show();
      this.service.getResult("/RadicalC/GetAllradical")
    .subscribe(
      (data:any) => {
        this.result = [];
        this.result=data;
