...
import { CompteB } from 'src/app/Models/base/CompteB';
...
@Component({
  selector: 'app-comptebilant',
  templateUrl: './comptebilant.component.html',
  styleUrls: ['./comptebilant.component.scss']
})
export class ComptebilantComponent implements OnInit {
cols = [
     { field: 'comptebilanid',  header: 'Code', width: '20%', display: 'table-cell',  align: 'left', size: '0.8em',background: 'black',color: 'white'},
  ...
  imprimer() {
    ...
    this.Spinner.show();
      this.service.getList("/CompteB/Printreport", this.resultx)
    .subscribe(
      (data:any) => {
    ...
    for (let file of files)
      ...
      formData.forEach((value, key) => {
        console.log("key %s: value %s", key, value);
           ...
           const uploadReq = new HttpRequest('POST',this.service.BaseURI + `/CompteB/UploadFile3`, formData, {
              reportProgress: true,
            });
...
this.displaycompte = true;
  ...
  selectrow(e) {
      this.angForm.setValue({ name: e.data.libelle, code: e.data.comptebilanid });
      this.codeid =e.data.comptebilanid;
    this.nameElement.nativeElement.focus();
  }
  onDelete(){
   ...
   res.push('id'); res.push(this.codeid);
    this.resultx = []; this.resultx = res;
   this.Spinner.show();
     this.service.getList("/CompteB/GetDelcomptebilan", this.resultx)
   .subscribe(
     (data:any) => {
  ...
  onSubmit() {
    this.loading = true; let fiile = new CompteB(); let i = 0;
      fiile.libelle = this.angForm.value.name;
       fiile.comptebilanid = this.angForm.value.code;

      this.service.ExecutePost("/CompteB/Update", fiile).subscribe(() => {
      if (i === 0) {
