...
export class TypecompteComponent implements OnInit {
  cols = [
    ...
    { field: 'libelle', header: 'Libelle', width: '80%', display: 'table-cell',  align: 'left', size: '0.8em',background: 'black',color: 'white',colorheader: 'green !important'},
...
this.displaycompte = true;
  ...
  onDelete(){
   ...
   res.push('id'); res.push(this.codeid);
    this.resultx = []; this.resultx = res;
   this.Spinner.show();
     this.service.getList("/Typecompte/GetDeltypecompte", this.resultx)
   .subscribe(
     (data:any) => {
  ...
  onSubmit() {
    this.loading = true; let fiile = new typecomptem(); let i = 0;
      ...
      this.service.ExecutePost("/Typecompte/Update", fiile).subscribe(() => {
      if (i === 0) {
