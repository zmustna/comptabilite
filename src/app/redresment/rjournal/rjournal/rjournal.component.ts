...
@Component({
  selector: 'app-rjournal',
  templateUrl: './rjournal.component.html',
  styleUrls: ['./rjournal.component.css']
})
export class RjournalComponent implements OnInit {
  ...
  cols = [
    ...
    { field: "mv_libop", header: "Libelle",  width: "45%", display: "table-cell",  align: "left", size: "0.8em" },
    ...
    { field: "", header: "Supprimer",  display: "none",  align: "left", size: "0.8em" },
  ];
  colsusercompany = [
...
display:boolean=false;moiss:number=0;journn:number=0;wmois:number=0;wccode:number=0;
...
onReset(){
  this.wmvtmvt=0;this.wmvtmvt1 =0;
  this.wmois = 0; this.moiss=0; this.wjour = 0; this.journn=0;this.wjourn=0; this.wjourn1=0;this.result = []; this.angForm.reset(); this.wligne = 0;this.wfolioelement.nativeElement.value = ' ';
  this.wfolio1element.nativeElement.value = ' ';this.wccode=0;
}
  getmvtdetail(){
  ...
  onSubmit() {
    this.Spinner.show();
 let res = new Array<any>();
 res.push('wjourealfrom');  res.push(this.journn);
 res.push('wjournalto');  res.push(this.journn1);
 res.push('wfoliofrom');  res.push(this.wmvtmvt);
 res.push('wfolioto');  res.push(this.wmvtmvt1);
res.push('company'); res.push( parseInt( localStorage.getItem("companycode")));
res.push('wyear'); res.push(parseInt( localStorage.getItem("exercicecompany")));
 this.resultx = []; this.resultx = res;
 this.service.getList("/fmvt/Getupdatejournal", this.resultx)
 .subscribe(
  (data:any) => {
   this.onReset();
    this.Spinner.hide();
